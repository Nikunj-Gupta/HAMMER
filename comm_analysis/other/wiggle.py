...
def run(args): 
    ...
    for _ in range(10000):
        obs['agent_0'][W1] = np.random.uniform(-2, 2)
        obs['agent_0'][W2] = np.random.uniform(-2, 2)

        actions, messages = HAMMER.policy_old.act(obs, HAMMER.memory, HAMMER.global_memory)  
        
        x.append(HAMMER.global_memory.states[0][0, W1])
        y.append(HAMMER.global_memory.states[0][0, W2])
        message_index = 0 
